...
DevInfo_t devinfo;
  ...
  {
	  printf("set china mobile GSM para\r\n");
	  g_OMCSetParaFlag = 0;
    ...
    if (g_DevType == MAIN_UNIT)
    {
		...
		if (DbGetThisIntPara(BINDWIDTH_MODEL, &val) == 1){ //宽带选频切换
			set_bandwidth_model(val);
		}
		//TD第一转换点微调
...
**
*/
void set_bandwidth_model(int model)
{
	if(model==1){
		drv_write_fpga(0x73,1);
	}else{
		drv_write_fpga(0x73,0);
	}
}
