...
int bat_error_flag;		//泰尔测试电池故障延时计数器
...
{
	int ret_b;
	ret_b = get_outside_error();	
	if((ret_b&0x1) == 1)
	{
		if(bat_error_flag<90)	//180为20分钟
		{
			bat_error_flag++;
			printf("\n---------------------------\n");
			printf("bat_error_alarm_delay= %d\n",bat_error_flag);
		}
		else
		{
			return 1;
...
void actual_alarm(void)
{
	...
	for(i = 1; i < 5; i++){	//泰尔测试将外部告警1改为电池故障告警
...
#define ALARM_CNT 	13 
#define RECOVER_CNT 28  //22 
#define ALARM_CNT_CS 30 
...
//	if((bat_error_alarm_cnt > ALARM_CNT) && ((val1&0x1) == 1))
	...
	else if(((bat_error_all_cnt-bat_error_alarm_cnt) > RECOVER_CNT))	//|| ((val1&0x1) == 0)){ // 告警恢复
	{ 
		bat_error_alarm_cnt = 0;
		bat_error_all_cnt = 0;
	}else if (((val1&0x1) == 0)){
	...
	if(g_DevType == MAIN_UNIT){
		if(((pw_down&0x1) != 1) && ((pw_error&0x1) != 1)){
			if((get_net_group()&0xf00) == 0x300){
				...
				DbGetThisIntPara(LTE1_DIN_OVER_EN_ID, &val1); // 使能
			}
