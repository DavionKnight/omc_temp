  ...
  if (g_OMCSetParaFlag==1)
  {
    ...
    if (g_DevType == MAIN_UNIT)
    {
		...
		if(get_net_group() != 0x331){
			...
			{
				f1 = (float)val; 
				f2 = 11828*exp(0.0114*f1);  // f2 = 11610*exp(0.01156*f1);开启门限的传输比是10
			...
			{
				f1 = (float)val; 
				f2 = 6940*exp(0.0108*f1);	//f2 = 11610*exp(0.01156*f1);开启门限的传输比是10
		...
		if((get_net_group() == 0x331)){ // 电信
			...
			*/
			...
			else if((val >= 39650) && (val<=41589))// 工作频段D tdd
			{
				val = 2496000 + 100*(val-39650) - 92160;//92.16MHz频点偏移
				dru_lmx2531_lte1_config(val);
			}
  ...
  if ((time(NULL)-m_query_time) > 2)//告警查询计时,间隔6s
  {
	  ...
	  if (g_DevType == MAIN_UNIT)
	  {
		  ...
		  if(get_net_group() == 0x331){
			  ...
			  DbSaveThisIntPara_MCP_C(GSM_MAX_DELAY_ID, val+18000, 1);
		  ...
		  else
		  {
			  ...
			  if((m_lte_num&0x1) == 0)
			  {
				  ...
				  if(((para2&0x1) == 0) || (val == 2))
				  { // 失步 通道选择2
					  DbSaveThisIntPara_MCP_C(LTE1DLPOWER_ID, -40, 1);
				  }
				  else
				  { 
				  	  drv_read_fpga(LTE1DATAPOWER_ADDR, &para2);	//使用LTE数字功率通过计算得出
					  para2 &= 0x1ff; 
					  val2 = para2/2 -36;
					  DbGetThisIntPara(LTE1DLPOWEROFFSET_ID, &val);
					  val2 = val2 + val/10 - 10; 
			  ...
			  else
			  {
				  ...
				  {
				  	  drv_read_fpga(LTE2DATAPOWER_ADDR, &para2);	//使用LTE数字功率通过计算得出
					  para2 &= 0x1ff; 
					  val2 = para2/2 -36;
					  DbGetThisIntPara(LTE2DLPOWEROFFSET_ID, &val);
					  val2 = val2 + val/10 - 10; 
...
//判断频点是否重复
int is_valid_channel(int num, int * channel)
{
	int i = 0;
...
{	
	...
	//配置DDC
	drv_write_ddc(11, val);	//40m使用第12信道做为测试通道
	//配置DUC
		...
		if(val&0x1 == 1)			//自动载波跟踪开关打开
		{
			...
			while((stop_time-start_time) < 300)
			{
				...
				if(end_flag==1)
				{
					...
					{
						printf("channel_num=%d\n.", tbuf[i]);
						if(1 == is_valid_channel(tbuf[i], channel_buf))
						{
							channel_buf[valid_idx++] = tbuf[i];
						}
					}
					memset((void *)tbuf, 0, 64);
