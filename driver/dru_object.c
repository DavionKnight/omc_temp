...
//const char * g_ver = "DRU-V002.0";
// UD:联通DCS UG：联通GSM MD：移动DCS MG：移动GSM
const char * g_type_ver = "UG"; 
const char * g_drv_ver = "10";
const char * g_test_ver = "2"; // 0:正式版 奇数：带测试版 偶数：已测试版本
...
void dru_init(void)
{
	...
	printf("config power sw!/r/n");
	power_sw_init();
	printf("power int ok!/r/n");
	power_sw_open();
	printf("power open ok!/r/n");
#ifdef CONFIG_KSZ8873
	dru_ksz8873_init();
 	printf("configure ksz8873\n");
#endif
 #ifdef CONFIG_LMH6517
	//dru_lmh6517_init();
 #endif
#ifdef CONFIG_AD9523
...
//	dru_lmx2531_init();
//dru_lmx2531_wcdma_init(2252840);
//dru_lmx2531_lte2_config(2252840);
//dru_lmx2531_lte2_init();

	if(get_device_type()==DEVICE_TYPE_MAIN){
		dru_lmx2531_wcdma_config(1983900);// main
	}else{
		dru_lmx2531_wcdma_config(1855340); //RAU
	}
//
dru_lmx2531_gsm_init();

//dru_lmx2531_fdd_lte1_init();
//dru_lmx2531_fdd_lte2_init();

if(get_device_type()==DEVICE_TYPE_MAIN){
	dru_lmx2531_fdd_lte1_config(1711400);// main
	dru_lmx2531_fdd_lte2_config(1711400);// main
}else{

	dru_lmx2531_fdd_lte1_config(1677840); // RAU
	dru_lmx2531_fdd_lte2_config(1677840);
}

//printf("configure lmx2531_gsm\n");
...
spi_delay(1000);
*DRU_REGADDR(fpga_base_addr, CPU_AGC_EN)= 0;  
//dru_lte1_gain_set(30);
//gain_test = dru_lte1_gain_read();

//dru_lte2_gain_set(13);
//gain_test = dru_lte2_gain_read();

//dru_gsm_gain_set(13);
//gain_test = dru_gsm_gain_read();

//dru_td_gain_set(13);
//gain_test = dru_td_gain_read();
#endif
