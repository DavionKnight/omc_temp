...
static int ks8851_probe(void)
{
	...
	ks->netdev = netdev;
	ks->hw_addr = ioremap(0x66000000, 32);
	printk("hw_addr=0x%08x.\n", ks->hw_addr);
	if (!ks->hw_addr)
		goto err_ioremap;

	printk(KERN_INFO "2\n");
	//ks->hw_addr_cmd = ioremap(0x66000004, 4);
	ks->hw_addr_cmd = ks->hw_addr + 8; 
	printk("hw_addr_cmd=0x%08x.\n", ks->hw_addr_cmd);
	if (!ks->hw_addr_cmd)
		goto err_ioremap1;
...
*/
	...
	//ks_read_config(ks);
	while(1){
	tmp = ks_rdreg16(ks, KS_CIDER);
	printk("chip id = 0x%04x.\n", tmp);
	}
	printk(KERN_INFO "6\n");
	/* simple check for a valid chip being connected to the bus */
	if (ks_rdreg16(ks, KS_CIDER)  != CIDER_ID) {
...
//	struct resource *iomem = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	...
	printk(KERN_INFO "remove ks8851\n");
...
static int ks8851_init(void)
{
	...
	addr = ioremap(0x66000000, 8);
	iowrite16(0x30c0, addr+8);
	tmp = ioread16(addr);
	printk("ksz8851 chip id = 0x%04x\n", tmp);
	return 0;
}
static int ks8851_exit(void)
{
 return 0;
}
