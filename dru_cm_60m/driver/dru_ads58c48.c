...
int dru_ads58c48_init(void)
{
	...
	emif_epld_adc_spi_write(0X2628,ADS58C48_REG_LENGTH);   // CHB SNRBoost FILter
	emif_epld_adc_spi_write(0X3228,ADS58C48_REG_LENGTH);   // CHC SNRBoost FILter
...
int dru_ads58c48_init_2581(void)
{
	//spi_channel(SELECT_ADS58C48);
	//usleep(1000);
	emif_epld_adc_spi_write(0X0002,ADS58C48_REG_LENGTH);
	...
	emif_epld_adc_spi_write(0X4400,ADS58C48_REG_LENGTH);   // DIGITAL MODE2 EN=0
	emif_epld_adc_spi_write(0X4200,ADS58C48_REG_LENGTH);   // 可调整接口输出时钟和数据的相位关系
	emif_epld_adc_spi_write(0X2D28,ADS58C48_REG_LENGTH);   // CHA SNRBoost FILter  0x28 = 40M 带宽  0x1E = 20M 带宽
